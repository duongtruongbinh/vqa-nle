...
SYSTEM_PROMPTS = """<image>Bạn là hệ thống Visual Question Answering (VQA). Nhiệm vụ của bạn là trả lời các câu hỏi dựa trên nội dung của hình ảnh được cung cấp."""
USER_PROMPTS = """
Câu hỏi: {question}
    Vui lòng trả lời câu hỏi sau dựa trên hình ảnh. Hãy trả lời theo định dạng sau:
...
def create_jsonl_for_msswift(split="train", output_file=None, image_base_dir="/mnt/VLAI_data/COCO_Images"):
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for idx, item in enumerate(raw_data):
            ...
            full_response = f"<answer>{answer}</answer><think>{explanation}</think>"
