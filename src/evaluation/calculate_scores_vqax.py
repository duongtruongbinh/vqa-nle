...
from .text_preprocessing import normalize_answer_vqax, normalize_explanation, ensure_list, clean_text
from .nlg_metrics import get_nlg_scores


YES_SET = {"yes", "true", "correct", "có", "đúng", "vâng"}
NO_SET = {"no", "false", "incorrect", "không", "sai"}
FILES_TO_EVALUATE = ['vintern3br_vqax_1000steps_result.json']
...
def compute_smile_max_ref(questions, gt_answers_list, predictions, synthetic_answers=None):
    ...
    for i, (q, refs, syn_ans, pred) in enumerate(zip(questions, gt_answers_list, synthetic_answers, predictions)):
        ...
        q_seg, syn_ans_seg, pred_seg = clean_text(q), clean_text(syn_ans), clean_text(pred)
        if not q_seg or not syn_ans_seg or not pred_seg:
            continue
        
        for ref in refs:
            ref_seg = clean_text(normalize_answer_vqax(ref))
            if ref_seg:
    ...
    max_avg_scores, max_hm_scores = {}, {}
    for k, sample_idx in enumerate(sample_mapping):
        s_avg, s_hm = results_flat['avg'][k], results_flat['hm'][k]
        max_avg_scores[sample_idx] = max(max_avg_scores.get(sample_idx, s_avg), s_avg)
        max_hm_scores[sample_idx] = max(max_hm_scores.get(sample_idx, s_hm), s_hm)
    
    if not max_avg_scores:
        return {"SMILE_avg": 0.0, "SMILE_hm": 0.0}

    return {
        "SMILE_avg": float(np.mean(list(max_avg_scores.values()))) * 100,
        "SMILE_hm": float(np.mean(list(max_hm_scores.values()))) * 100
    }


def extract_gt_answers(item):
    gt_answers_multiref = []
    gt_answers_unsorted = []
    ...
    return gt_answers_multiref, gt_answers_unsorted


def check_accuracy_multiref(pred_ans, gt_answers_multiref, gt_answers_unsorted, raw_pred):
    ...
    for gt in gt_answers_multiref:
        if gt in YES_SET and not c_pred_tokens.isdisjoint(YES_SET):
            return True
        if gt in NO_SET and not c_pred_tokens.isdisjoint(NO_SET):
            return True
...
def evaluate_file(json_path: str, device: str = "cuda", use_synthetic_answers: bool = False) -> dict:
    ...
    all_gt_answers_list = []
    all_primary_gt_answers = []
    by_type = {}
    
    for item in data:
        total += 1
        gt_answers_multiref, gt_answers_unsorted = extract_gt_answers(item)
        
        raw_pred = item.get("predict", "")
        pred_ans = normalize_answer_vqax(raw_pred)
        ...
        if check_accuracy_multiref(pred_ans, gt_answers_multiref, gt_answers_unsorted, raw_pred):
            correct += 1
    ...
    if use_synthetic_answers:
        ...
        all_synthetic_answers = SharedSyntheticAnswerGenerator.generate_batch(
            questions=all_questions, answers=all_primary_gt_answers,
    ...
    for ans_type, data_type in by_type.items():
        ...
        smile = compute_smile_max_ref(
            ...
            synthetic_answers=data_type.get("synthetic_answers")
        )
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate VQA-X predictions")
